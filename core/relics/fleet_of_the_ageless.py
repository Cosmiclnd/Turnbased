        ...
        def __init__(self, t, relic_set, pieces):
            ...
            if self.pieces >= 2:
                mod1 = modifier.Modifier(self.relic_set.nameid, self.relic_set.name,
                    modifier.StatDesc((None, None, relic_set.config.get_skill_value("2pc", "hp_boost"))),
                    None, t)
                ...
                t.stats["atk"].modifiers.append(mod2)
        
        def validator_mod2(self, stat, **kwargs):
            return stat.target.stats["spd"].calculate() >= self.relic_set.config.get_skill_value("2pc", "spd_threshold")
