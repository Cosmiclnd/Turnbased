...
class InertSalsotto(base.RelicSet):
    class PiecesEffect(base.RelicSet.PiecesEffect):
        def __init__(self, t, relic_set, pieces):
            ...
            if self.pieces >= 2:
                mod1 = modifier.Modifier(self.relic_set.nameid, self.relic_set.name, modifier.StatDesc((None, None, 0.08)), None, t)
                t.stats["crt_rate"].modifiers.append(mod1)
                mod2 = modifier.Modifier(self.relic_set.nameid, self.relic_set.name, modifier.StatDesc((None, None, 0.15)), self.validator_mod2, t)
        ...
        def validator_mod2(self, stat, **kwargs):
            ...
            if dmg is None:
                return False
            return stat.target.stats["crt_rate"].calculate() >= 0.5 and dmg.source in (damage.DmgSource.ULTIMATE, damage.DmgSource.FOLLOW_UP)
