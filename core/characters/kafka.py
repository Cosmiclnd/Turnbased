...
class Kafka(base.Character):
    class BasicAtk(base.Character.CharacterSkill):
        ...
        async def skill_trigger(self, skill):
            ...
            if self.target.eidolons >= 1:
                await self.target.dot_dmg_vulnerability_eidolon1((t,))
    ...
    class Skill(base.Character.CharacterSkill):
        ...
        async def skill_trigger(self, skill):
            if self is not skill:
                return
            if self.target.eidolons >= 1:
                await self.target.dot_dmg_vulnerability_eidolon1([self.get_main_target()] + self.get_adjacent_targets())
            await battle.current.event_bus.dispatch("attack_start", self.target)
            t = self.get_main_target()
            dmg = await damage.Damage.create(self.target, t,
            ...
            for t in self.get_adjacent_targets():
                ...
                await battle.current.event_bus.dispatch("hit", dmg)
            await battle.current.event_bus.dispatch("attack_end", self.target)
            await battle.current.event_bus.dispatch("tick_dot",
                damage.DotTick(self.get_main_target(), lambda x: True, self.get_value("main_dot_tick_percentage")))
            for t in self.get_adjacent_targets():
                await battle.current.event_bus.dispatch("tick_dot", damage.DotTick(t, lambda x: True, self.get_value("sub_dot_tick_percentage")))
    ...
    class Ultimate(base.Character.CharacterUltimate):
        ...
        async def skill_trigger(self, skill):
            if self is not skill:
                return
            if self.target.eidolons >= 1:
                await self.target.dot_dmg_vulnerability_eidolon1(battle.current.monsters[:])
            await battle.current.event_bus.dispatch("attack_start", self.target)
            for t in battle.current.monsters[:]:
                ...
                dmg.toughness_reduction = damage.ToughnessReduction(self.get_value("toughness_reduction"), self.target.element)
                await battle.current.event_bus.dispatch("hit", dmg)
                await self.target.inflict_shock(t, self.get_value("duration"), self.get_value("base_chance"))
            await battle.current.event_bus.dispatch("attack_end", self.target)
            for t in battle.current.monsters[:]:
                await battle.current.event_bus.dispatch("tick_dot", damage.DotTick(t, lambda x: True, self.get_value("dot_tick_percentage")))
            await battle.current.event_bus.dispatch("energy_regen", self.target, self.get_value("energy_regen"))
    ...
    class Talent(base.Character.CharacterSkill):
        ...
        async def skill_trigger(self, skill):
            ...
            if self.target.eidolons >= 1:
                await self.target.dot_dmg_vulnerability_eidolon1((self.skill_target,))
            await battle.current.event_bus.dispatch("attack_start", self.target)
            dmg = await damage.Damage.create(self.target, self.skill_target,
                modifier.StatDesc((self.target.stats["atk"], modifier.ModifierFilter.CALCULATED, self.get_value("percentage"))),
            ...
            for ratio in (0.15, 0.15, 0.15, 0.15, 0.15, 0.25):
                dmg.hit_split_ratio = ratio
                await battle.current.event_bus.dispatch("hit", dmg)
                await self.target.inflict_shock(self.skill_target, self.get_value("duration"), self.get_value("base_chance"))
            await battle.current.event_bus.dispatch("attack_end", self.target)
            if self.target.traces_unlocked[2]:
                await battle.current.event_bus.dispatch("tick_dot", damage.DotTick(self.skill_target, lambda x: True,
    ...
    async def clean(self, t):
        ...
        if context.effect is not None and context.effect.is_debuff_type(effect.Debuff.SHOCK):
            await battle.current.event_bus.dispatch("regen_energy", self, self.config.get_skill_value("bonus_trace2", "energy_regen"))
    
    @event.member_listener(event.ListenerPriority.PRE_PROCESS, "deal_damage")
    async def deal_damage_eidolon2(self, dmg):
    ...
    async def deal_damage_eidolon4(self, dmg):
        if not isinstance(dmg.target, monster.Monster) or not dmg.is_dot_dmg():
            return
        context = dmg.context
        if context.effect is not None and context.effect.is_debuff_type(effect.Debuff.SHOCK) and context.effect_instance.caster is self:
            await battle.current.event_bus.dispatch("regen_energy", self, self.config.get_skill_value("eidolon4", "energy_regen"))
    
    async def inflict_shock(self, t, duration, base_chance):
        if self.eidolons >= 6:
        ...
        await self.try_apply_debuff(eff_add, base_chance)
    
    async def dot_dmg_vulnerability_eidolon1(self, targets):
        for t in targets:
            ...
            await self.try_apply_debuff(eff_add, self.config.get_skill_value("eidolon1", "base_chance"))
